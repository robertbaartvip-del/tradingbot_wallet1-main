...
class LongPositionBehavior(PositionBehavior):
    ...
    def check_for_sl(self, position, row):
        return row['close'] <= position.sl_price

    def check_for_tp(self, position, row):
        return row['close'] >= position.tp_price
...
def update_equity_record(
    ...
    if time - previous_equity_update_time >= equity_update_interval:
        ...
        filter = {}
        if not len(trades_info) == 0:
            df = pd.DataFrame(trades_info)
            df['close_time'] = pd.to_datetime(df['close_time'])
            filter = df[df['close_time'].dt.date == pd.Timestamp(time - equity_update_interval).date()]
            # raise ValueError(f"{len(filter[filter['open_reason'].str.contains("long", case=False, na=False)])}")
        ...
        equity_record.append({
            ...
            "price": price,
            "equity": equity,
            "total_trades": len(filter),
            "long_trades": 0 if len(filter) == 0 else len(filter[filter['open_reason'].str.contains("long", case=False, na=False)]),
            "short_trades": 0 if len(filter) == 0 else len(filter[filter['open_reason'].str.contains("short", case=False, na=False)]),
            "win_trades": 0 if len(filter) == 0 else len(filter[filter['initial_margin'] <= filter['close_balance']]),
            "lose_trades": 0 if len(filter) == 0 else len(filter[filter['initial_margin'] > filter['close_balance']]),
            "initial_balance": 0 if len(filter) == 0 else int(filter['close_balance'].iloc[0]),
            "final_balance": 0 if len(filter) == 0 else int(filter['close_balance'].iloc[-1]),
            "profit_or_loss": 0 if len(filter) == 0 else float(filter['close_balance'].iloc[-1]) - float(filter['close_balance'].iloc[0])
        })
